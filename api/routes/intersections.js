...
router.get('/', async (req, res) => {
    try {
        const cacheKey = 'intersections:all';
        ...
        const [rows] = await db.pool.execute(`
            SELECT id, name, latitude, longitude, status, created_at, updated_at
            FROM intersections
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const [trafficLightRows] = await db.pool.execute(`
            SELECT id, direction, phase_number, current_status, 
        ...
        const result = {
            intersection: intersectionRows[0],
...
router.post('/', async (req, res) => {
    try {
        const { name, latitude, longitude, status = 1 } = req.body;
        ...
        const [result] = await db.pool.execute(`
            INSERT INTO intersections (name, latitude, longitude, status)
            VALUES (?, ?, ?, ?)
        ...
        // 清除相关缓存
        await redisCfg.setCache('intersections:all', null, 0);
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (!name && !latitude && !longitude && status === undefined) {
            return res.status(400).json({
                success: false,
                message: '缺少更新参数'
            });
        ...
        // 清除相关缓存
        await redisCfg.setCache('intersections:all', null, 0);
        await redisCfg.setCache(`intersection:${id}`, null, 0);
